...
// Chakra imports
import { Box, Button, Flex, Grid, HStack, Icon, Link, Menu, MenuItem, MenuList, MenuButton, Stack, Text, useColorModeValue, useColorMode, useDisclosure, SimpleGrid } from '@chakra-ui/react';
import IconBox from '@/components/icons/IconBox';
import SidebarResponsive from '@/components/sidebar/SidebarResponsive';
import { AiFillStar } from 'react-icons/ai';
import { GoChevronDown, GoChevronRight } from 'react-icons/go';
import { NavLink } from 'react-router-dom';
import routes, { MenuItem as IMenuItem } from '@/variables/menu';
import { Provider } from './context/SidebarWidthContext';
...
const NavbarExample: React.FC<NavbarExampleProps> = ({ children, logo, logoText, secondary, sidebarWidth, ...rest }) => {
	...
	const { isOpen: isOpenApplication, onOpen: onOpenApplication, onClose: onCloseApplication } = useDisclosure();

	const { isOpen: isOpenEcommerce, onOpen: onOpenEcommerce, onClose: onCloseEcommerce } = useDisclosure();
	...
	const createPagesLinks = (routes: IMenuItem[]) => {
		return routes.map(link => {
			...
			if (link.name === 'Pricing Page') {
				return (
					<Stack direction="column" key={link.name}>
			...
			if (link.authIcon) {
				return (
					<Stack direction="column" key={link.name}>
						<Stack direction="row" spacing="6px" align="center" mb="6px" cursor="default">
							<IconBox bg="blue.500" color="white" h="30px" w="30px" icon={link.authIcon} />
							<Text fontWeight="bold" fontSize="sm" color={textColor}>
								{link.name}
							</Text>
						</Stack>
				...
				if (link.component) {
					return (
						<NavLink to={link.layout + link.path}>
							<MenuItem ps="36px" py="0px" _hover={{ boxShadow: 'none', bg: 'none' }} borderRadius="12px">
	...
	const createAuthLinks = (routes: IMenuItem[] | undefined) => {
		if (!routes) return;
		return routes.map(link => {
			if (link.authIcon && link.collapse === true) {
				return (
					<Stack direction="column" my="auto" key={link.name}>
				...
				return (
					<NavLink to={link.layout + link.path} key={link.name}>
	...
	const linksAuth = (
		<HStack display={{ sm: 'none', lg: 'flex' }} spacing="12px">
			<Stack direction="row" spacing="4px" align="center" color="#fff" fontWeight="bold" onMouseEnter={onOpenPages} onMouseLeave={onClosePages} cursor="pointer" position="relative">
				<Text fontSize="sm" color={mainText}>
					Pages
				</Text>
				<Icon as={GoChevronDown} color={mainText} w="14px" h="14px" fontWeight="2000" />
				<Menu isOpen={isOpenPages}>
					<MenuList bg={menuBg} p="22px" minW="550px" cursor="default" borderRadius="15px" position="absolute" top="30px" left="-10px">
			...
			<Stack direction="row" spacing="4px" align="center" color="#fff" fontWeight="bold" onMouseEnter={onOpenAuth} onMouseLeave={onCloseAuth} cursor="pointer" position="relative">
				<Text fontSize="sm" color={mainText}></Text>
				<Icon as={GoChevronDown} color={mainText} w="14px" h="14px" fontWeight="2000" />
				<Menu isOpen={isOpenAuth}>
					<MenuList bg={menuBg} p="22px" minW="450px" cursor="default" borderRadius="15px" position="absolute" top="30px" left="-10px">
						<Stack direction="row" spacing="24px">
							<Flex
								...
								borderRadius="15px"
							>
								<IconBox bg="white" color="white" borderRadius="50%" h="50px" w="50px" mb="12px" icon={<Icon as={AiFillStar} w="25px" h="25px" color="blue.500" />} />
			...
			<Stack direction="row" spacing="4px" align="center" color="#fff" fontWeight="bold" onMouseEnter={onOpenApplication} onMouseLeave={onCloseApplication} cursor="pointer" position="relative">
				<Text fontSize="sm" color={mainText}>
					Application
				</Text>
				<Icon as={GoChevronDown} color={mainText} w="14px" h="14px" fontWeight="2000" />
				<Menu isOpen={isOpenApplication}>
					<MenuList bg={menuBg} p="22px" cursor="default" borderRadius="15px" position="absolute" top="30px" left="-10px">
			...
			<Stack direction="row" spacing="4px" align="center" color="#fff" fontWeight="bold" onMouseEnter={onOpenEcommerce} onMouseLeave={onCloseEcommerce} cursor="pointer" position="relative">
				<Text fontSize="sm" color={mainText}>
					Ecommerce
				</Text>
				<Icon as={GoChevronDown} color={mainText} w="14px" h="14px" fontWeight="2000" />
				<Menu isOpen={isOpenEcommerce}>
					<MenuList bg={menuBg} p="22px" minW="350px" cursor="default" borderRadius="15px" position="absolute" top="30px" left="-10px">
